...
import { getPortalLabOrder, type PatientLabOrderFull } from '@/lib/portal/lab-portal-api'
...
type ResultStatus = 'PENDING' | 'NORMAL' | 'HIGH' | 'LOW' | 'CRITICAL_HIGH' | 'CRITICAL_LOW'
...
function interpLabel(status: ResultStatus) {
...
function interpColor(status: ResultStatus) {
...
function valColor(status: ResultStatus) {
...
function LabReport({
  order, patientName, generatedAt,
}: {
  order: PatientLabOrderFull
  patientName: string
  generatedAt: string
}) {
    ...
    <div style={{ fontFamily: "'Arial', 'Helvetica Neue', sans-serif", fontSize: '10.5pt', color: '#1e293b', lineHeight: 1.4 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9.5pt' }}>
            ...
            <tbody>
              {results.map((r, i) => {
                ...
                const status = r.status as ResultStatus
                const ast = asterisk(status)
                const bg = rowBg(status)
                return (
                  <tr key={i} style={{ background: bg ?? (i % 2 !== 0 ? '#fafafa' : undefined) }}>
      ...
      <div style={{ marginTop: 20, paddingTop: 10, borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div style={{ fontSize: '7.5pt', color: '#94a3b8', lineHeight: 1.7 }}>
          ...
          <div>Solo para uso del paciente y el personal médico autorizado.</div>
        </div>
        <div style={{ textAlign: 'right' }}>
...
export default function PortalLabPrintPage() {
  const { id } = useParams<{ id: string }>()
  const [order, setOrder] = useState<PatientLabOrderFull | null>(null)
  ...
  useEffect(() => {
    ...
    const name = `${session.patient.first_name} ${session.patient.last_name}`
    setPatientName(name)
    getPortalLabOrder(session.token, id).then(o => {
      setOrder(o)
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          ...
          #portal-lab-print-overlay { visibility: visible; position: static !important; overflow: visible !important; background: white !important; }
          #portal-lab-print-overlay * { visibility: visible; }
          .no-print { display: none !important; }
          @page { size: letter portrait; margin: 12mm 15mm; }
        }
      ` }} />

      <div id="portal-lab-print-overlay">
        {/* Screen controls */}
        ...
        <div style={{ padding: '32px 24px 56px', minHeight: 'calc(100% - 57px)' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', paddingTop: '6rem', color: '#94a3b8', fontSize: 14 }}>Informe no disponible.</div>
          ) : (
            <div style={{
              ...
              <LabReport order={order} patientName={patientName} generatedAt={generatedAt} />
            </div>
          )}
        </div>
      </div>
